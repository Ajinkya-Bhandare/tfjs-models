...
function detectPoseInRealTime(video, net) {
  ...
  async function poseDetectionFrame() {
    ...
    var pos;
    var len_x,len_y;
    var left = [3,5,11,13,15] ; //Calculate left length of a figure(ear,ear,shoulder,hip,knee,ankle)
    var right = [4,6,12,14,16] ; // Calculate right length of figure(ear,ear,shoulder,hip,knee,ankle)
      ...
      if (score >= minPoseConfidence) {
        for (i=0;i<keypoints.length;i++){
            ...
            if (keypoints[i].score<thresh_score){
              console.log(false);
              //alert("please take a step back so that we can see your entire pose"); //Generate alert to capture full pose
            }
         }
